...
function buildRegistrationCompleteEmbed(data, slotNumber, group, scrimName, store) {
  ...
  // Custom message set via the admin panel (see admin-panel-handlers.js)
  // replaces the default footer line entirely, if one's been configured.
  const customMessage = store && store.settings && store.settings.registerConfirmationMessage;
  ...
  return new EmbedBuilder()
    ...
    .setFooter({ text: customMessage || 'Best of luck 👊 for your matches!!' });
}
