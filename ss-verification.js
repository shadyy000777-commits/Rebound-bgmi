...
async function analyzeOneScreenshot(imageUrl, instagramUsername) {
  ...
  // Unlike Groq, Gemini can't fetch a Discord CDN URL server-side — the
  // image has to be sent as inline base64 data, so we download it first.
  ...
  try {
    ...
    console.error('[ss-verify] Failed to download screenshot for Gemini:', err);
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [
            { text: buildPrompt(instagramUsername) },
    ...
    if (!text) {
      // Most common cause: the response was cut off by maxOutputTokens or
      // blocked by a safety filter before any text was produced.
    ...
    // already, but pull out just the {...} object as a safety net in case
    ...
    try {
      ...
      const finishReason = data?.candidates?.[0]?.finishReason;
      console.error(`[ss-verify] Failed to parse JSON from vision model (finishReason: ${finishReason}):`, text);
