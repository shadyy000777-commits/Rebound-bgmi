...
const {
  handleAdminPanelButton, handleAdminPanelChannelSelect,
  handleAdminPanelRoleSelect, handleAdminPanelModalSubmit,
...
const { handleSsVerifyMessage } = require('./ss-verification');
...
client.on('messageCreate', async (message) => {
  ...
  // Instagram-follow screenshot verification: anything posted in the
  // channel configured via /set-ss-verify-channel gets treated as a
  // verification submission, not a normal chat message — checked before
  // the AI-mention handling below so a screenshot post there never
  // accidentally falls through to the AI instead.
  const ssStore = getGuildStore(message.guild.id);
  if (ssStore.settings && ssStore.settings.ssVerifyChannelId === message.channelId) {
    try {
      await handleSsVerifyMessage(message);
    } catch (err) {
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.customId.startsWith('admin_panel_role_select:')) {
        await handleAdminPanelRoleSelect(interaction);
      }
      ...
      } else if (interaction.customId.startsWith('admin_panel_channel_select:')) {
        await handleAdminPanelChannelSelect(interaction);
      }
      ...
      else if (interaction.customId.startsWith('admin_panel_modal:')) await handleAdminPanelModalSubmit(interaction);
    }
  } catch (err) {
