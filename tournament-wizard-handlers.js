...
async function handleTourneyRegConfirm(interaction) {
  ...
  // Registration itself only grants the tournament-wide success role.
  // Role failures are logged (not swallowed silently)
...
async function handleQualifySelect(interaction) {
  ...
  if (promoted.length && !tournament.round2RoleId) lines.push('ℹ️ No Round 2 role is set — set one via Edit Settings → H. Round 2 Role.');
...
module.exports = {
  ...
  handleCreateRound2RoleSelect,
  handleRound2GroupSizeModalSubmit,
