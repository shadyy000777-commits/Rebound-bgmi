...
function buildTournamentWizardPayload(store) {
  ...
  const row1 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId('tourney_wizard_manage_slot').setLabel('Manage Slot').setEmoji('🧩').setStyle(ButtonStyle.Primary),
  );
...
// View Groups / Slot List / Qualify used to live here too, but each
// group's own channel now carries its own admin panel (Publish Slot List /
// Punish Team / Result — see buildTournamentGroupAdminPanelPayload) which
// covers those jobs scoped to that specific group, so they were removed
// from here.
...
function buildCreateChannelsSubmenuPayload() {
  const embed = new EmbedBuilder()
    ...
    .setColor(0x5865F2)
    .setDescription(
      '**Auto Channels** creates every group\'s channel right away with the default naming (`group-<letter>`), under an auto-managed "🥇 Tournament Groups" category.\n\n' +
      '**Create Channel** lets you pick the category and naming format yourself first.'
    );
...
function buildManageGroupsSubmenuPayload(tournament) {
  const embed = new EmbedBuilder()
    ...
    .setColor(0x5865F2)
    .setDescription('Groups are now auto-created from Total Slots / Teams-per-Group when you hit **Create Channels** — nothing to set up here manually. Each group\'s own channel carries its own Publish Slot List / Punish Team / Result panel.');

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tourney_wizard_manage_groups_action').setLabel('Group Tools').setEmoji('🗂️').setStyle(ButtonStyle.Secondary),
  );
...
function buildTournamentGroupAdminPanelPayload(letter) {
  const embed = new EmbedBuilder()
    ...
    .setColor(0x5865F2)
    .setDescription('**Publish Slot List** posts this group\'s current teams here. **Punish Team** bans a registered team and strips their roles. **Result** marks this group\'s qualifiers.');
...
// Sub-panel behind "Manage Slot" — the deliberate admin step that hands out
// each group's role/channel to teams that are already sitting in that
// group's slot list. Registration itself only grants the tournament-wide
// success role now (see handleTourneyRegConfirm) — this button is what
// actually turns "registered" into "has their group's channel access".
function buildManageSlotSubmenuPayload(tournament) {
  const capacity = tournament.teamsPerGroup || DEFAULT_GROUP_CAPACITY;
  const embed = new EmbedBuilder()
    .setTitle('🧩 Manage Slot')
    .setColor(0x5865F2)
    .setDescription(
      `**Auto Setup** works through groups in slot order — Group 1's first ${capacity} registered teams get ` +
      `Group 1's role and channel, then once Group 1 is full it moves on to Group 2, and so on. ` +
      `Only players who still hold the success role get their group role; anyone who lost it is skipped.`
    );

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tourney_wizard_auto_setup').setLabel('Auto Setup').setEmoji('⚙️').setStyle(ButtonStyle.Success),
  );
...
function buildHelpEmbed() {
  return new EmbedBuilder()
    ...
    .setDescription([
      ...
      '**Manage Groups** — group tools (being reworked)',
      ...
      '**Manage Slot** — Auto Setup hands out each group\'s role/channel to already-registered teams, 20 slots (or your configured group size) at a time, group by group',
...
// Total Slots / Teams-per-Group if none exist yet. Returns an error string
// to show the admin, or null once groups are ready.
function ensureGroupsExist(interaction, store) {
  ...
  const result = computeAutoGroups(tournament, tournament.totalSlots, perGroup);
...
// only the tournament's success role can see it, and it can't spin up
// threads — same lockdown regardless of which path made it.
...
function buildManualChannelCreationPayload(data) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription('Configure your tournament channels')
    .addFields(
      { name: 'Channel Format', value: data.channelFormat ? `\`${data.channelFormat}\`` : '`Not Set`' },
      { name: 'Category Name', value: data.categoryName ? `\`${data.categoryName}\`` : '`Not Set`' },
      { name: 'Status', value: ready ? '✅ Ready — hit Create Channels.' : '🔒 Set both format and category name to continue' },
    );

  const row1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tourney_manual_channels_set_format').setLabel('Channel Format').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('tourney_manual_channels_set_category').setLabel('Category Name').setStyle(ButtonStyle.Primary),
  );
  const row2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tourney_manual_channels_create').setLabel('Create Channels').setEmoji('✅').setStyle(ButtonStyle.Success).setDisabled(!ready),
    new ButtonBuilder().setCustomId('tourney_manual_channels_cancel').setLabel('Cancel').setEmoji('🚫').setStyle(ButtonStyle.Danger),
  );
...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id === 'tourney_wizard_manage_slot') {
    if (!store.tournament) {
      return interaction.reply({ content: '❌ No tournament exists yet.', flags: MessageFlags.Ephemeral });
    }
    return interaction.reply({ ...buildManageSlotSubmenuPayload(store.tournament), flags: MessageFlags.Ephemeral });
  }

  if (id === 'tourney_wizard_auto_setup') {
    return handleTourneyAutoSetup(interaction, store);
  }
  ...
  if (id === 'tourney_wizard_create_channels_auto') {
    ...
    if (!store.tournament.successRoleId) {
      return interaction.reply({
        content: '❌ Set a success role first — Edit Settings → C. Success Role — so I know who to let into these private channels.',
        flags: MessageFlags.Ephemeral,
      });
    }
  ...
  if (id === 'tourney_manual_channels_set_format') {
    ...
    const input = new TextInputBuilder()
      .setCustomId('value')
      .setLabel('Use {letter} for the group letter')
      .setStyle(TextInputStyle.Short)
      .setPlaceholder('group-{letter}')
  ...
  if (id === 'tourney_manual_channels_set_category') {
    const data = getPendingChannelCreation(interaction.guildId, interaction.user.id) || {};
  ...
  if (id === 'tourney_manual_channels_create') {
    ...
    try {
      category = await interaction.guild.channels.create({
        ...
        type: ChannelType.GuildCategory,
        reason: `Tournament group category created by ${interaction.user.tag}`,
      });
    } catch (err) {
    ...
    const created = await createGroupChannels(interaction, store, { nameFormat: data.channelFormat, parentId: category.id });
    clearPendingChannelCreation(interaction.guildId, interaction.user.id);
    return interaction.editReply({
      content: created.length
        ? `✅ Created category **${category.name}** with: ${created.join(', ')}`
        : 'ℹ️ Category created, but every group already has a channel (or channel creation failed — check my permissions).',
      embeds: [],
      components: [],
    });
  }
  ...
  if (id === 'tourney_wizard_manage_groups_action') {
    if (!store.tournament) {
      return interaction.reply({ content: '❌ No tournament exists yet.', flags: MessageFlags.Ephemeral });
    }
    // Placeholder — behavior for this button to be defined next.
    return interaction.reply({ content: '🚧 This button isn\'t wired up yet — tell me what it should do.', flags: MessageFlags.Ephemeral });
  }
...
async function handleTournamentCreateModalSubmit(interaction) {
  ...
  store.tournament = {
    ...
    successRoleId: null, requiredMentions: 4, teamsPerGroup: DEFAULT_GROUP_CAPACITY, totalSlots: null,
    reactions: ['✅', '❌'],
  };
  saveGuildStore(interaction.guildId, store);
...
async function ensureTournamentGroupChannelAndRole(interaction, store, groupKey) {
  ...
  if (!role) {
    if (!botMember.permissions.has('ManageRoles')) {
      ...
      console.error(`[tournament-group-role] Guild ${interaction.guildId} is at/near Discord's ${MAX_GUILD_ROLES}-role cap — skipping auto-create for Group ${groupKey}.`);
    } else {
      try {
        ...
        console.error(`[tournament-group-role] Failed to auto-create role for Group ${groupKey} in guild ${interaction.guildId}: ${err.code ?? ''} ${err.message}`);
      }
    }
  }
  ...
  if (!group.channelId || !interaction.guild.channels.cache.has(group.channelId)) {
    ...
    // to @everyone and now also grants this group's role the same private,
    // no-threads access, instead of staying keyed to only the success role.
...
async function handleTourneyRegConfirm(interaction) {
  ...
  // Registration itself only grants the tournament-wide success role now —
  // the per-group role (and its channel) is handed out later in a
  // deliberate batch by an admin via Manage Slot → Auto Setup, not
  // automatically here. Role failures are logged (not swallowed silently)
  // and surfaced in the confirmation embed so an admin doesn't have to dig
  // through logs to notice registrations aren't getting the role.
  ...
  if (tournament.successRoleId) {
    ...
    for (const userId of roleTargets) {
      ...
      await member.roles.add(tournament.successRoleId).catch(err => {
        failures++;
        console.error(`[tournament-success-role] Failed to add role ${tournament.successRoleId} to ${userId} in guild ${interaction.guildId}: ${err.code ?? ''} ${err.message}`);
      });
    }
    if (failures) {
      roleWarning = `\n\n⚠️ Couldn't give the success role to ${failures === roleTargets.size ? 'everyone' : `${failures} player(s)`} — ask an admin to check my **Manage Roles** permission and that my role sits above <@&${tournament.successRoleId}>.`;
    }
  } else {
    console.warn(`[tournament-success-role] No successRoleId configured for guild ${interaction.guildId} — set one via Edit Settings → C. Success Role.`);
  }
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          ...
          `Your group's channel and role will be handed out once an admin runs **Manage Slot → Auto Setup**.` +
          (roleWarning || '')
        ),
    ],
    components: [],
...
// "Manage Slot" → "Auto Setup" — the deliberate step that hands out each
// group's role (creating its role/channel the first time, same helper the
// old auto-on-registration path used) to the teams already sitting in that
// group's slot list. Groups are worked in slot order (Group 1, then 2, ...)
// which — since autoAssignGroup fills a group to its capacity (20 by
// default) before moving to the next one — is exactly "first 20
// registrations get Group 1, next 20 get Group 2, and so on". Only players
// who still hold the tournament's success role get their group role;
// anyone who lost it since registering (banned, left, manually stripped)
// is silently skipped.
async function handleTourneyAutoSetup(interaction, store) {
  if (!store.tournament) {
    return interaction.reply({ content: '❌ No tournament exists yet.', flags: MessageFlags.Ephemeral });
  }
  const tournament = store.tournament;
  if (!tournament.successRoleId) {
    return interaction.reply({
      content: '❌ Set a success role first — Edit Settings → C. Success Role.',
      flags: MessageFlags.Ephemeral,
    });
  }

  const letters = Object.keys(tournament.groups)
    .filter(l => tournament.groups[l].teams.length)
    .sort((a, b) => Number(a) - Number(b));
  if (!letters.length) {
    return interaction.reply({ content: 'ℹ️ No registered teams to set up yet.', flags: MessageFlags.Ephemeral });
  }

  await interaction.deferReply({ flags: MessageFlags.Ephemeral });

  let groupsSet = 0, rolesGiven = 0, alreadyHad = 0, notEligible = 0, missingMember = 0;
  const failedGroups = [];
  ...
  for (const letter of letters) {
    ...
    if (!groupRole) {
      failedGroups.push(letter);
      continue;
    }
    groupsSet++;

    for (const team of group.teams) {
      const targets = new Set([team.ownerId, ...(team.playerIds || [])]);
      for (const userId of targets) {
        const member = interaction.guild.members.cache.get(userId)
          ?? await interaction.guild.members.fetch(userId).catch(() => null);
        if (!member) { missingMember++; continue; }
        if (!member.roles.cache.has(tournament.successRoleId)) { notEligible++; continue; }
        if (member.roles.cache.has(groupRole.id)) { alreadyHad++; continue; }
        await member.roles.add(groupRole.id).catch(() => {});
        rolesGiven++;
      }
    }
  }
  ...
  const lines = [
    `✅ Set up **${groupsSet}**/${letters.length} group${letters.length === 1 ? '' : 's'} with registrations.`,
    `🎯 Gave out **${rolesGiven}** group role${rolesGiven === 1 ? '' : 's'}.`,
  ];
  if (alreadyHad) lines.push(`↪️ ${alreadyHad} already had their group role.`);
  if (notEligible) lines.push(`⏭️ ${notEligible} skipped — no longer holding the success role.`);
  if (missingMember) lines.push(`⚠️ ${missingMember} couldn't be found in the server.`);
  if (failedGroups.length) {
    lines.push(`❌ Couldn't create a role/channel for Group${failedGroups.length === 1 ? '' : 's'} ${failedGroups.join(', ')} — check my **Manage Roles**/**Manage Channels** permissions and the server's role/channel limits.`);
  }
...
async function handleCreateRoleSelect(interaction) {
  ...
  // Same checks as /set-register-role — catching these here means the
  // admin finds out immediately, instead of registrations silently
  // failing to hand out the success role later with no visible error.
  if (role.managed) {
    return interaction.reply({
  ...
  store.tournament.successRoleId = role.id;
  saveGuildStore(interaction.guildId, store);
  await interaction.update({ content: '', ...buildCreateSettingsPayload(store.tournament) });
}
  ...
  for (const team of punishedTeams) {
    ...
    for (const userId of targets) {
      const member = interaction.guild.members.cache.get(userId)
        ?? await interaction.guild.members.fetch(userId).catch(() => null);
      if (!member) continue;
      if (tournament.successRoleId) await member.roles.remove(tournament.successRoleId).catch(() => {});
