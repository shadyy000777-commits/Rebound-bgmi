...
async function createGroupChannels(interaction, store, { nameFormat, parentId }) {
  ...
  for (const letter of letters) {
    ...
    const role = await ensureGroupRole(interaction, store, group, `Group ${letter}`, `Auto-created for Group ${letter} tournament registration`);
...
async function ensureTournamentGroupChannelAndRole(interaction, store, groupKey) {
  ...
  const role = await ensureGroupRole(interaction, store, group, `Group ${groupKey}`, `Auto-created for Group ${groupKey} tournament registration`);
