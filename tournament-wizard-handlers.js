...
function buildTournamentGroupAdminPanelPayload(tournament, roundNum, letter) {
  ...
  const resultDescription = isFinalRound
    ? '**Result** marks this group\'s final qualifiers — this is the last configured round, so nobody gets promoted further.'
...
function buildQualifySelectPayload(tournament, roundNum, letter) {
  ...
  const select = new StringSelectMenuBuilder()
    ...
    .setPlaceholder(`Select qualifying teams from ${label}`)
    .setMinValues(0)
  ...
  const maxRounds = getMaxRounds(tournament);
  const embed = new EmbedBuilder()
    .setTitle(`✅ Qualify Teams — ${label}`)
    .setColor(0x5865F2)
    .setDescription(
      roundNum >= maxRounds
        ? 'Select every team from this group that qualifies, then confirm. This is the final configured round, so picks are marked as qualifiers but nobody is promoted further.'
...
async function handleQualifySelect(interaction) {
  ...
  // this round is the last one Manage Rounds is configured for.
  const promoted = [];
  const failed = [];
  if (nextRound <= maxRounds) {
  ...
  if (nextRound > maxRounds && selectedNames.length) lines.push(`ℹ️ Round ${roundNum} is the final configured round — these teams are marked as qualifiers, not promoted further. Raise **Manage Rounds** → Max Rounds to chain another round.`);
