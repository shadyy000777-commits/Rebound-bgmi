...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id === 'tourney_wizard_create_channels_manual') {
    ...
    if (!store.tournament.successRoleId) {
      return interaction.reply({
        content: '❌ Set a success role first — Edit Settings → C. Success Role — so I know who to let into this private channel.',
        flags: MessageFlags.Ephemeral,
      });
    }
  ...
  if (id === 'tourney_wizard_manual_channels_create') {
    ...
    // Exactly one category, one channel — not per group. Private the same
    // way as every other tournament channel: only the success role can see
    // it, and it can't spin up threads.
    let category;
    try {
      category = await interaction.guild.channels.create({
    ...
    try {
      channel = await interaction.guild.channels.create({
        ...
        permissionOverwrites: [
          { id: interaction.guild.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel] },
          {
            id: store.tournament.successRoleId,
            allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory],
            deny: [PermissionFlagsBits.CreatePublicThreads, PermissionFlagsBits.CreatePrivateThreads],
          },
        ],
