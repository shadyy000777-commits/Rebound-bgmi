...
function buildHelpEmbed() {
  return new EmbedBuilder()
    ...
    .setDescription([
      ...
      '**Create Channels** — the first press auto-creates groups from Total Slots / Teams-per-Group, then creates each group\'s channel',
...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id === 'tourney_wizard_create_channels') {
    ...
    // Groups are no longer created by hand — the first time Create Channels
    // is pressed with no groups yet, auto-generate them straight from the
    // Total Slots / Teams-per-Group settings (Edit Settings), same math the
    // old Auto Groups modal used, just without asking for numbers again
    // since they're already configured.
    if (!Object.keys(store.tournament.groups).length) {
      if (!store.tournament.totalSlots) {
        return interaction.reply({
          content: '❌ Set **Total Slots** first — Edit Settings → Total Slots (and Teams-per-Group, if you want something other than the default).',
          flags: MessageFlags.Ephemeral,
        });
      }
      const perGroup = store.tournament.teamsPerGroup || DEFAULT_GROUP_CAPACITY;
      const result = computeAutoGroups(store.tournament, store.tournament.totalSlots, perGroup);
      if (result.error) {
