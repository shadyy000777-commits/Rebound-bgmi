...
function buildTournamentWizardPayload(store) {
  ...
  const row1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setLabel(tournament.open ? 'Start/Pause Reg' : 'Start/Pause Reg')
      .setEmoji(tournament.open ? '⏸️' : '▶️')
...
function buildHelpEmbed() {
  return new EmbedBuilder()
    ...
    .setDescription([
      ...
      '**Register Team** — public button, teams register themselves and get auto-assigned',
...
function buildAutoGroupsModal() {
  return new ModalBuilder()
    .setCustomId('tourney_wizard_auto_groups_modal')
    .setTitle('Auto-Create Groups')
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('total').setLabel('Total teams expected').setStyle(TextInputStyle.Short)
          .setPlaceholder('e.g. 64').setRequired(true).setMaxLength(4)
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('per_group').setLabel('Teams per group').setStyle(TextInputStyle.Short)
...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id === 'tourney_wizard_edit_settings') {
    ...
    return interaction.showModal(buildEditSettingsModal(store.tournament));
  }
  ...
  if (id === 'tourney_wizard_auto_groups') {
    ...
    return interaction.showModal(buildAutoGroupsModal());
  }
...
async function handleTournamentCreateModalSubmit(interaction) {
  ...
  store.tournament = { name, open: false, groups: {}, qualified: [], bannedTeams: [], slotManagerChannelId: null };
  saveGuildStore(interaction.guildId, store);
