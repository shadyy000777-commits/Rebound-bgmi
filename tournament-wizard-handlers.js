...
async function handleTourneyRegConfirm(interaction) {
  ...
  // Auto-creates (once) a role + channel scoped to this group, and gives
  // that role to the registrant plus everyone they mentioned — same idea
  // as the tournament-wide success role below, just scoped per group.
  const groupRole = await ensureTournamentGroupChannelAndRole(interaction, store, letter).catch(() => null);

  // The success role(s) go to the player who registered plus everyone they
  // mentioned as part of the lineup — never lets a role hiccup block the
  // registration itself.
  if (tournament.successRoleId || groupRole) {
    const roleTargets = new Set([interaction.user.id, ...selectedPlayerIds]);
    for (const userId of roleTargets) {
      const member = interaction.guild.members.cache.get(userId)
        ?? await interaction.guild.members.fetch(userId).catch(() => null);
      if (!member) continue;
      if (tournament.successRoleId) await member.roles.add(tournament.successRoleId).catch(() => {});
      if (groupRole) await member.roles.add(groupRole.id).catch(() => {});
    }
  }
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          ...
          (groupRole ? `\n**Group Channel** — <#${tournament.groups[letter].channelId}>` : '')
        ),
    ],
    components: [],
  });
