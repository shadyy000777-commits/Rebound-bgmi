...
function buildManageGroupsSubmenuPayload(tournament) {
  const embed = new EmbedBuilder()
    ...
    .setColor(0x5865F2)
    .setDescription('Add a group, or auto-create a batch of them. Each group\'s own channel carries its own Publish Slot List / Punish Team / Result panel.');
...
function buildHelpEmbed() {
  return new EmbedBuilder()
    ...
    .setDescription([
      ...
      '**Manage Groups** — add groups, or auto-create a batch of them',
...
async function handleTournamentWizardButton(interaction) {
  ...
  if (id === 'tourney_wizard_add_group') {
    ...
    if (Object.keys(store.tournament.groups).length >= GROUP_LETTERS.length) {
      return interaction.reply({ content: `❌ Max ${GROUP_LETTERS.length} groups reached.`, flags: MessageFlags.Ephemeral });
    }
...
async function handleAutoGroupsModalSubmit(interaction) {
  ...
  if (!Number.isInteger(total) || total < 1 || total > MAX_TOTAL_SLOTS) {
    return interaction.reply({ content: `❌ Total teams must be a whole number between 1 and ${MAX_TOTAL_SLOTS}.`, flags: MessageFlags.Ephemeral });
  }
