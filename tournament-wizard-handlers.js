...
// created via "Create Channels" — the per-group counterpart to the
// top-level wizard panel, scoped to just this one group.
...
// Creates (once per Round 2 group) a role + private channel together, the
// moment a team is promoted via "Result" — unlike Round 1 registration,
// promotion is an admin action, not self-service, so there's no need to
// hold the channel back separately from the role here. Scoped to its own
// category/roles/channels so it never collides with Round 1's.
